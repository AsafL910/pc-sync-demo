...
inject_env() {
  info "Generating .env from node values..."
...
start_node_a() {
  ...
  docker compose up -d --build postgres nats mission_service bridge_service gps_service alerts_service gps_sim frontend
...
start_node_b() {
  ...
  docker compose up -d --build postgres_b nats_b mission_service_b bridge_service_b gps_service_b alerts_service_b gps_sim_b frontend_b
...
test_alert_recovery() {
  ...
  info "Powering off Node B services (docker compose stop)..."
  docker compose stop postgres_b nats_b mission_service_b bridge_service_b gps_service_b alerts_service_b gps_sim_b frontend_b > /dev/null 2>&1
  ...
  info "Restarting Node B services (with --build)..."
  docker compose up -d --build postgres_b nats_b mission_service_b bridge_service_b gps_service_b alerts_service_b gps_sim_b frontend_b

  info "Waiting for Node B to catch up..."
  sleep 15

  info "Checking Node B NATS logs for the alert msg..."
  if docker logs nats_b 2>&1 | grep -q "$ALERT_MSG"; then
    pass "Node B retrieved missing alert from Node A JetStream!"
  else
    fail "Node B did NOT retrieve the missing alert"
    docker logs nats_b | tail -n 20
  fi
}
